---
title: "Chapter 14: Supervised Classification"
...
needs(discrim, glmnet, textrecipes, tidymodels, tidytext, tidyverse, workflows, naivebayes, themis)
...
```{r}
split <- initial_split(imdb_data, prop = 0.1, strata = sentiment)
...
imdb_basic_recipe <- recipe(sentiment ~ text, data = imdb_train) |> 
  ...
  # additional pre-processing steps can be added, see next sections
...
Now that the data is ready, the model can be specified. The `parsnip` package is used for this. It contains a model specification, the type, and the engine. For Naïve Bayes, this would look like the following (note that you will need to install the relevant packages -- here: `naivebayes` -- before using them):
...
-   Logistic regression (penalized with Ridge):
...
The predictions can also be used to create the confusion matrix by hand.
...
lambda_grid <- grid_regular(
  penalty(range = c(-4, 0)), # gets log10 transformed into log_10(-4) = 0.0001, log_10(-2) = 0.01, log_10(0) = 1
...
tune_lasso_rs <- read_rds("https://www.dropbox.com/scl/fi/q5xn17yl9x4gblnjpqb15/tuned_lasso.rds?rlkey=rbebd56a30fpvxzwnhpjzkak3&dl=1")
```
...
timelines_gb |> count(party)

split_gb <- initial_split(timelines_gb, prop = 0.1, strata = party)
party_tweets_train_gb <- training(split_gb)
party_tweets_test_gb <- testing(split_gb)

party_tweets_train_gb |> count(party)
```
