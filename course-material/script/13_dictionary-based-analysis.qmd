---
title: "Chapter 13: Dictionary-based Analysis"
...
The AFINN dictionary is the only one with numeric values. You might have noticed that its words are not stemmed. Hence, I need to do this before I can join it with my tibble. Then, to get the sentiment value per document, I can simply average it.
...
Now we can calculate precision (when it predicts "positive", how often is it correct), recall/sensitivity (when it is "positive", how often is this predicted), specificity (when it's predicted "negative", how often is it actually negative). The [F1-score](https://deepai.org/machine-learning-glossary-and-terms/f-score) is the harmonic mean of precision and recall and defined as $F_1 = \frac{2}{\frac{1}{recall}\times \frac{1}{precision}} = 2\times \frac{precision\times recall}{precision + recall}$ and the most commonly used measure to assess the accuracy of the classification. The closer to 1 it is, the better. You can find a more thorough description of the confusion matrix and the different measures in [this blog post](https://machinelearningmastery.com/confusion-matrix-machine-learning/).
